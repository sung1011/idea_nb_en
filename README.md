...
- **单词图鉴**：格子展示动物岛 15 词 + `-ap` / `-an` 槽位；已解锁显示 emoji + 英文，未解锁剪影问号；点已解锁词会 TTS 朗读。关卡里点对 / 翻对 / 点对地鼠 / 拖对 / 钓到 / 跟读通过即解锁，逛图鉴不加当日星星
- **动物岛大厅**：今日目标「帮小猫把 -at 朋友请来派对！」；进度 0/2；「完整一日」走原两关
- **玩法一览**：列出 7 种玩法，点进 demo 试玩，不加当日星星
- **读词钓鱼 / Word Fish**：每局从 15 词抽出 3 条单词鱼；读出单词即挂钩吊进网里，点鱼或点喇叭也能玩；试玩不加星
- **Echo Cave**：每局抽 3 词；TTS 范读后跟读；有 `SpeechRecognition` 就听，没有或失败可点「我说好了」
- **Flash Flip 闪卡翻翻**：每局抽 4 词，先翻卡看图+词并听读（约 30–40 秒热身），再听词点对；试玩不加星，点对解锁图鉴
- **Whack Word 地鼠词**：每局抽 5 词；地鼠弹出图词，听目标词点对的那只；点对 4 次过关，点错轻晃不倒计时卡死；试玩不加星，点对解锁图鉴
- **Drag Sort 拖一拖**：每局抽 3 词，把纯英文单词芯片拖到对应图片篮子（篮子无中文，芯片无图；英语 TTS 演示）
- **Sing Along 唱一唱**：每局抽 3 词，两句 chant 高亮，点「我唱好了」（不测音高）
- **Find Scene 找一找**：派对场景用 Pixi 画布点出本局 3 个词（Vue 壳保留；试玩不加星）
