...
- **首页**：主按钮「去动物岛」；标题下是今日目标条（一条主任务 + 可选焦点词 + 今日星星条 空/实 4 星）；次入口「玩法一览」；弱入口「字母工坊 / 复习音族」「单词图鉴」
- **单词图鉴**：格子展示动物岛 15 词 + `-ap` / `-an` 槽位；已解锁显示描边软陶词卡 + 英文，未解锁剪影问号；点已解锁词会 TTS 朗读。关卡里点对 / 翻对 / 点对地鼠 / 拖对 / 钓到 / 跟读通过即解锁，逛图鉴不加当日星星
- **动物岛大厅**：岛名下是今日目标条（默认「今天走完派对四关」，完成打勾；有焦点词时「多听一听 cat」；下方今日星星条）；进度 0/4，每关旁一颗空/实星；「今日主线」按 `chainStep` 进入当天第一关（偶数日闪卡翻翻，奇数日地鼠词）
...
- **本地存储**：统一键 `starWords.v2`（当日星星 / 终身星星 / 贴纸 / 图鉴解锁 / 动物岛 0–7 日 / 当日任务）。旧键 `starWords.v1` 与 `starWords.atlas.v1` 会在首次读取时迁入。日期按 Asia/Shanghai 日历日重置当日进度。主线关首次通关 +1 当日星；重玩、`?demo=1`、`?review=1`、找一找 / 唱一唱不加当日星
