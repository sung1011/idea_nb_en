...
解锁：课内关卡严格按顺序；上一课全通才开下一课；一章 4 课都通才开下一章。**不用等日历日**。上海 `dateKey` 只用于文案 / 分析。没有 `levels` 的课显示「即将开放」，点了不会进游戏、也不会被当成已通关。当前第 1 章、第 2 章和第 3 章各四课有内容（每课 5 关：闪卡、地鼠、听音拼一拼、小书点读、小小回顾）。一章最后一课的回顾关带该章徽章（`rise1` / `rise2` / `rise3`「词族和字母徽章」）。第 4–12 章仍是「即将开放」。数学课只标记 `type: 'math'`，这一期不做数字关。
...
src/meadow/                    星星草地（名册、存档、舞台、孵化、喂食）。一起玩 / hearts / 饰品仍留接口；饰品图已在 `public/meadow/`
...
- **存档**：`meadow` 挂在现有 `starWords.v2` 上，不升 persist 版本。记下拥有的动物、待孵的蛋、每只脚的 x/y 百分比、家长开关、当天过关次数。`hearts` 先存着不用。吃饱打盹只记在内存里。喂到会调用 `onFeed(animalId, foodId, isFavorite)`，好让下一阶段加爱心
...
- **还没做**：一起玩、爱心计数、饰品（`acc-hat` / `acc-bow` / `acc-scarf` 的图已经放好）
