...
- **小商店和装饰**：草地上方有「小商店」和可以花的星星数。底栏 `shopSheet.vue` 按 `MEADOW_DECORATIONS` 列卡片（图、中文名、价格 ⭐）。买得起就扣 `spentStars`，装饰从上方落到草地上并弹一下，配一声开心的音效。星星不够时卡片用淡蓝色写「还差 N 颗星」，按钮不可点，没有红色，也没有失败弹窗。每种只能买一次，买过的写「已拥有」。装饰可以拖到别的位置，松手就记住，下次进来还在。现在上架的是水池（10 星，`deco-pond`）和火堆（15 星，`deco-campfire`）。饿着的动物不会去玩装饰，仍耷拉着，直到喂饱。不饿的空闲动物大约每 20–40 秒会走向一件已有的装饰玩一会儿：水池边喝水，火堆旁边取暖。把动物拖到水池上会在水里扑腾，溅出水滴，气泡说 “Splash!” 并读出来，几秒后跳出来。拖到火堆上会站在火堆旁边（不会站进火里），举着棉花糖，说 “So warm!”。点水池有涟漪和水声，点火堆有噼啪声和几点火花，火苗会轻轻忽闪。同一时间只读一句英文。装饰表在 `meadowConfig.ts`：id、中文名、价格、图片、互动类型、英文短句。互动类型已经有 water、fire、swing、ball、flower、house。之后加秋千、皮球、花坛、小屋，只要在这张表加一行，并放一张透明、底对齐的 webp 到 `public/meadow/`
