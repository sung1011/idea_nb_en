...
解锁：课内关卡严格按顺序；上一课全通才开下一课；一章 4 课都通才开下一章。**不用等日历日**。上海 `dateKey` 只用于文案 / 分析。没有 `levels` 的课显示「即将开放」，点了不会进游戏、也不会被当成已通关。当前第 1 章到第 12 章各四课有内容。第 1–4 章、第 5 章后三课、第 6 章、第 7 章、第 8 章后三课、第 9 章、第 10 章前三课、第 11 章和第 12 章后三课的单词课每课 5 关：闪卡、地鼠、听音拼一拼、小书点读、小小回顾。数字课是第 5 章第 1 课（课17：50–100）、第 7 章第 2 课（课26：6–9）和第 8 章第 1 课（课29：0、11–13）和第 10 章第 4 课（课40：5、10、15、20）和第 12 章第 1 课（课45：4、14、16、18），五关为数字闪卡、听音点数字、数一数、打地鼠数字、小小回顾。一章最后一课的回顾关带该章徽章（`rise1` / `rise2` / `rise3`「词族和字母徽章」/ `rise4`「母鸡徽章」/ `rise5`「小屋徽章」/ `rise6`「丹和卡姆徽章」/ `rise7`「义卖徽章」）。第 4 章四课的回顾关另外各发一枚课徽章（`rise13`「母鸡会跳徽章」/ `rise14`「字母 G 徽章」/ `rise15`「大力一击徽章」/ `rise16`「大胡萝卜徽章」）。第 5 章四课回顾各发 `rise17`「数字徽章」/ `rise18`「书桌帐篷徽章」/ `rise19`「小屋朋友徽章」/ `rise20`「看一看徽章」。第 6 章四课回顾各发 `rise21`「能蘸吗徽章」/ `rise22`「帽子在哪徽章」/ `rise23`「红色果酱徽章」/ `rise24`「心情徽章」。第 7 章四课回顾各发 `rise25`「拿到啦徽章」/ `rise26`「多一个徽章」/ `rise27`「我得到徽章」/ `rise28`「一起玩徽章」。第 8 章四课回顾各发 `rise29`「少一个徽章」/ `rise30`「蓝色徽章」/ `rise31`「坐下徽章」/ `rise32`「跳起来徽章」。第 9 章四课回顾各发 `rise33`「好一个徽章」/ `rise34`「跑过去徽章」/ `rise35`「她说徽章」/ `rise36`「停下徽章」。第 10 章四课回顾各发 `rise37`「哦一只徽章」/ `rise38`「爸爸爱做饭徽章」/ `rise39`「复习徽章」/ `rise40`「拍手徽章」。第 11 章四课回顾各发 `rise41`「字母 E 徽章」/ `rise42`「点一点 E 徽章」/ `rise43`「往前走徽章」/ `rise44`「给我看徽章」。第 12 章四课回顾各发 `rise45`「跳着数徽章」/ `rise46`「翻煎饼徽章」/ `rise47`「给我一个徽章」/ `rise48`「再见徽章」。12 章 48 课都有关卡。
...
3. **数一数**（`/number-count`）：0 显示空盘子，点盘子读 zero；1–20 用一颗颗星星或苹果；更大的数用十条一组（50 是 5 根、100 是 10 根，不能整除时再补个位数）。点每一根或每一颗会读出累计的英文（ten / twenty / … / one hundred）。再从这一课的四个数字里点对的。
4. **打地鼠数字**：地鼠关，洞里是数字。
...
src/composables/useSentenceAtlas.ts 句子图鉴只读视图（按课是否已过）
src/data/atlasCatalog.ts     两本图鉴共用的分章词表和句子
...
src/meadow/                    星星草地（名册、存档、舞台、孵化、喂食、一起玩、爱心、饰品、跳舞、小商店、装饰）
src/views/homeView.vue         首页（跨章目标条 + 当前章亮格 + 去动物岛 + 星星草地 + 贴纸相册 + 下面一排小按钮单词图鉴 / 句子图鉴 + 页底玩法一览 + 设置）
...
src/views/wordAtlasView.vue    单词图鉴（按章 + 草地动物）
src/views/sentenceAtlasView.vue 句子图鉴（按章，章内按课；未通关的课句子锁住）
...
- 通关后回该章列表会亮多一格；设置「初始化」后回到新玩家起点：前 `DEFAULT_COMPLETED_CHAPTERS` 章已过，当前停在下一章第 1 课
- 全清时整条变暖色，旁注「第N章通关啦」
...
- 设置「初始化」后相册只留下新玩家起点里已通关章的徽章，后面收集的贴纸会清掉
- 只看、不装饰小岛、不交换、不花费

## 星星草地（孵化、闲逛、喂食、一起玩）

`/star-meadow` 全屏草地（Teleport 到 `body`，不受 430px 壳限制）。名册在 `src/meadow/meadowConfig.ts`，按章：bunny、leopard、pig、hen、fox、bear、yak、frog、duck、ox、bird、panda。图在 `public/meadow/`（另有 `egg.webp`）。小动物只给好反馈，不会难过，也不会离开。大约 12 小时没喂会从饱变饿，孩子只能从耷拉的样子、以及会不会凑向食物看出来，界面上没有倒计时、数字或饥饿条。
...
- **孵化**：章通关（与章徽章同一判断）把该章动物排进 `pendingEggs`。第 4 章通关孵母鸡 hen，第 5 章通关孵狐狸 fox，第 6 章通关孵小熊 bear，第 7 章通关孵牦牛 yak，第 8 章通关孵青蛙 frog，第 9 章通关孵小鸭 duck，第 10 章通关孵小牛 ox，第 11 章通关孵小鸟 bird，第 12 章通关孵熊猫 panda。已经通关、这次才装上的章，下次进草地逐个孵。设置「设置当前课」跳过的章也排队，但不在庆祝页立刻孵
...
- **存档**：`meadow` 挂在现有 `starWords.v2` 上，不升 persist 版本。记下拥有的动物、待孵的蛋、每只脚的 x/y 百分比、爱心、戴的饰品、每只的 `lastFedAt`、收进篮子时的 `storedAt`（0 表示还在草地上，旧档没有就当 0）、时钟高水位 `clockMark`、调试用的 `hungerSkipMs`、花掉的星星 `spentStars`（旧档没有就当 0）、已买的装饰 `{ id, x, y, stored? }`、家长开关、当天过关次数。可以花的星星是 `lifetime.totalStars - spentStars`。终身星星只增不减，首页星星条仍显示 `lifetime.totalStars`，买装饰不会让它变少。连吃 3 口后的短打盹仍只记在内存里。喂到会调用 `onFeed(animalId, foodId, isFavorite)`，并把这一只的 `lastFedAt` 拨到当前有效时间。旧档没有 `lastFedAt` 时，按这次读档的时刻当作刚喂饱，不会一打开就全饿；新孵出来的也是饱的。设置「初始化」回到新玩家起点：前 `DEFAULT_COMPLETED_CHAPTERS` 章的小动物已经在草地上、按当前时间刚喂饱，不播孵化；装饰、`spentStars` 和后面的动物清掉。设置「完全化」把每只爱心设成 5，饰品和跳舞都解锁，但不帮孩子选好饰品，也不送商店装饰；新放上的动物是饱的，本来就在的保留原来的 `lastFedAt`。设置里的「草地时间 +12小时」把饥饿时钟往前拨 12 小时。「星星 +50」走 `addStar(50)`，只加终身星星。设备时间往回拨时饱腹不会变多：经过的时间按不小于 0 算，并记住见过的最晚时刻
- **小商店和装饰**：草地上方有「小商店」和可以花的星星数。底栏 `shopSheet.vue` 按 `MEADOW_DECORATIONS` 列卡片（图、中文名、价格 ⭐）。买得起就扣 `spentStars`，装饰从上方落到草地上并弹一下，六件落点错开、不会叠在一起，配一声开心的音效。星星不够时卡片用淡蓝色写「还差 N 颗星」，按钮不可点，没有红色，也没有失败弹窗。每种只能买一次，买过的写「已拥有」。装饰可以拖到别的位置，松手就记住，下次进来还在。商店按价格从低到高排 6 件：皮球 5 星（`deco-ball`，大约动物头那么大）、花丛 5 星（`deco-flowers`，和水池差不多大）、秋千 10 星（`deco-swing`，约 1.3 倍动物）、水池 10 星（`deco-pond`）、火堆 15 星（`deco-campfire`）、小窝 20 星（`deco-house`，约 1.3 倍动物）。饿着的动物不会自己去玩装饰，仍耷拉着，直到喂饱。不饿的空闲动物大约每 20–40 秒会走向一件已有的装饰玩一会儿：水池边喝水，火堆旁边取暖，秋千上坐一会儿，花丛边闻一闻，偶尔把皮球轻轻顶一下；小窝自己去睡的很少，而且里面同时只能有一只。把动物拖到皮球上，它会先退半步再踢出去，说 “Kick!”。皮球高高抛起并旋转，地上的影子先缩小再变大，碰到草地边上会弹开，一共弹两三下、一次比一次低，落地时压扁并扬起一小团灰。小动物跟着球跑，停在球落下的旁边，新位置会记住。球飞过别的动物身边时，那只会高兴地跳一下。点一下皮球也会踢出去。拖到秋千上会越荡越高，一共三下，最后一下几乎到头顶，最高处说 “Wheee!”，然后飞出去，落地再弹两下，身边冒出小星星。拖到花丛会凑近闻一闻，停住，然后大打喷嚏，说 “Achoo!”，整个人向后翻一圈，花瓣飘过整片草地，花丛跟着晃；点花丛只飘几片花瓣和一声轻响。拖到水池会先跑两步、高高跃起再扎进去，说 “Splash!”，溅起很多水滴和一圈圈涟漪，再冒出来爬上岸甩水；溅到的旁边动物会跳一下。拖到火堆上火苗窜高、火星升起，脸颊变红，左右跟着火光晃，说 “So warm!”。拖进小窝会一头扎进去，说 “Good night!”，睡着时小窝一鼓一鼓、屋顶抬起落下，Zzz 越变越大；点它或大约 30 秒后弹出门，说 “Good morning!”，再大大伸个懒腰。点水池有涟漪和水声，点火堆有噼啪声和火花。自己走过去玩的动作比较轻。拖着小动物、它的中心进入一件装饰四周大约放大两成的范围时，这件装饰底下会出现慢慢呼吸的暖黄光圈，装饰弹到 1.1 倍并停住，上方有一个轻轻起伏的动作气泡（秋千弧线、小脚、花朵、水花、火苗、月亮和 Zzz）；动物旁边闪几颗小星星。刚进入范围会轻轻叮一声，设备支持的话短震一下。同时进入多件时只亮离中心最近的一件，离开范围马上熄灭。饿着的动物靠近时是灰色圈和苹果，用更轻的一声，松手不会开始玩。设置里的「装饰热区」把这些范围画成虚线。同一时间只读一句英文。拖小动物或装饰时，右下角会滑出一只藤编篮子，只有拖着的时候才看得到。拖到篮子上，盖子张开，底下亮起暖黄光圈并弹一下，刚进入会叮一声；如果篮子和装饰的放下范围叠在一起，优先收进篮子。松手后东西缩小掉进篮子，配一声轻轻的噗。草地右下角有一个小篮子按钮，不挡小商店，角上显示收着的件数，一件都没有时不显示数字。点开后是和商店一样的底栏，分成小动物和装饰两排，点一下就放回草地：装饰回到上次记住的位置，小动物从篮子里跳出来，落在收起前的位置附近。收起来的装饰仍算已拥有，不退星星。收起来的小动物不出现在草地上，也不会自己逛、玩装饰或冒出单词气泡；饥饿先停住，放出来时和收起时一样饱。新孵出来的小动物还是直接出现在草地上。这些都记在存档里，刷新还在；「初始化」会把篮子清空。装饰表在 `meadowConfig.ts`：id、中文名、价格、图片、`scale`、互动类型、英文短句
- **喂食**：动物栏上面有一排 6 种食物（fish / egg / bun / ham / fig / cake），拿不完。拖起来就用英文读出名字。放到动物身上（整张图再加一圈）会转向、咀嚼、掉碎屑、飘一颗心，并说 “Yum!”。最喜欢的食物反应更大，心会炸开，再说 “Yum! I love fig!”。喜欢的搭配写在配置里：bunny/yak/ox 喜欢 fig，leopard 喜欢 ham，pig/bird 喜欢 cake，hen/duck/panda 喜欢 bun，fox 喜欢 egg，bear/frog 喜欢 fish。放到别处会轻轻弹回盘子。大约 2 分钟内吃 3 口就躺下打盹 20 秒；这时候再喂，食物飘回去，动物晃一晃或轻轻哼一下，没有失败音。饱腹按真实时间从满线性减到饿，大约 12 小时（`MEADOW_HUNGER_MS`），用存下来的时间算，关掉或锁屏也会继续变。饿了会耷拉着，拖食物靠近时会走过来；还饱着就自己逛，不会凑过来。喂一口就重新吃饱。界面上不出现倒计时、数字或饥饿条
