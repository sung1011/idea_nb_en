...
- **存档**：`meadow` 挂在现有 `starWords.v2` 上，不升 persist 版本。记下拥有的动物、待孵的蛋、每只脚的 x/y 百分比、爱心、戴的饰品、每只的 `lastFedAt`、时钟高水位 `clockMark`、调试用的 `hungerSkipMs`、家长开关、当天过关次数。连吃 3 口后的短打盹仍只记在内存里。喂到会调用 `onFeed(animalId, foodId, isFavorite)`，并把这一只的 `lastFedAt` 拨到当前有效时间。旧档没有 `lastFedAt` 时，按这次读档的时刻当作刚喂饱，不会一打开就全饿；新孵出来的也是饱的。设置「初始化」把草地、爱心、饰品和饥饿时钟一起清空。设置「完全化」把每只爱心设成 5，饰品和跳舞都解锁，但不帮孩子选好饰品；新放上的动物是饱的，本来就在的保留原来的 `lastFedAt`。设置里的「草地时间 +12小时」把饥饿时钟往前拨 12 小时。设备时间往回拨时饱腹不会变多：经过的时间按不小于 0 算，并记住见过的最晚时刻
